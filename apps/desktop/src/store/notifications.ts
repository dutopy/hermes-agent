...
/** Profile-owned embedded prompt RPCs must not expose backend paths, tokens, or
 * raw transport detail. Legacy primary prompts preserve their existing detail. */
export function notifyPromptResponseError(error: unknown, fallback: string, ownerProfile?: null | string): void {
  notifyError(profiledPresentationError(error, fallback, ownerProfile), fallback)
}
