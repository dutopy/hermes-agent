...
from hermes_cli.kanban_present import (
    BOARD_COLUMNS,
    CARD_SUMMARY_PREVIEW_CHARS as _CARD_SUMMARY_PREVIEW_CHARS,
    task_dict as _task_dict,
    event_dict as _event_dict,
    comment_dict as _comment_dict,
    attachment_dict as _attachment_dict,
    run_dict as _run_dict,
    compute_task_diagnostics as _compute_task_diagnostics,
    warnings_summary_from_diagnostics as _warnings_summary_from_diagnostics,
    links_for as _links_for,
    parents_blocking_ready as _parents_blocking_ready,
    set_status_direct as _set_status_direct,
    resolve_project as _resolve_project_core,
    projects_by_id as _projects_by_id,
    board_counts as _board_counts,
    default_workspace_kind as _default_workspace_kind,
    validate_workdir as _validate_workdir_core,
)
...
# Core logic lives in hermes_cli.kanban_estimate (imported above as
# _run_estimate) so the dashboard and gateway RPC surface share it.
# ---------------------------------------------------------------------------
...
def _resolve_project(ref: Optional[str]) -> tuple[Optional[str], Optional[str], Optional[str]]:
    ...
    non-empty ref doesn't resolve to an existing project. Thin HTTP wrapper
    around ``kanban_present.resolve_project``, which raises plain
    ``ValueError`` (that module stays FastAPI-agnostic).
    """
    try:
        return _resolve_project_core(ref)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc))
...
def _validate_workdir(raw: str) -> str:
    ...
    paths — mirroring the create-board contract. Thin HTTP wrapper around
    ``kanban_present.validate_workdir``, which raises plain ``ValueError``.
    """
    try:
        return _validate_workdir_core(raw)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc))
